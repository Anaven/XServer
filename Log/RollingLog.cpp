...
void RollingLog::SetCloseOnWrite(bool closeOnWrite) 
{ 
    sCloseOnWrite = closeOnWrite; 
}

bool  RollingLog::IsLogEnabled() 
{ 
    return sCloseOnWrite || (fLog != NULL); 
}

void RollingLog::WriteToLog(char* inLogData, bool allowLogToRoll)
{
...
char* RollingLog::GetLogPath(const char *extension)
{
    char *thePath = NULL;
    
    char* logDir = this->GetLogDir(); //The string passed into this function is a copy
    char* logName = this->GetLogName();  //The string passed into this function is a copy
    
    char *formatPath = NEW char[::strlen(logDir) + ::strlen(logName) + ::strlen(extension) + 3]; //allocate the buffer
    ::sprintf(formatPath, "%s/%s", logDir, logName);

    if (extension)
        ::strcat(formatPath, extension);
...
void RollingLog::EnableLog( bool appendDotLog )
{
   //
    ...
    if (!logExists)
    {
       char* tempDir = this->GetLogDir();
       OS::RecursiveMakeDir(tempDir);
    }
...
void RollingLog::CloseLog(bool leaveEnabled)
{
    MutexLocker locker(&fMutex);
...
bool RollingLog::FormatDate(char *ioDateBuffer, bool logTimeInGMT)
{
...
bool RollingLog::CheckRollLog()
{
    ...
    {   
        ...
        RollingLog::ResetToMidnight(&fLogCreateTime, &logCreateTimeMidnight);
...
bool RollingLog::RenameLogFile(const char* inFileName)
{
    //returns false if an error occurred, true otherwise
    ...
    //AccessServer.981217003.log
    //format the new file name
    char *theNewNameBuffer = NEW char[::strlen(logDirectory) + kMaxFilenameLengthInBytes + 3];
    ...
    struct tm  timeResult;
    struct tm* theLocalTime = localtime_r(&fLogCreateTime, &timeResult);
    char timeString[10];
    ...
    for (SInt32 x = 0; (theErr == 0) && (x<=1000); x++)
    {
        ...
        WarnV((theErr == 0 || Thread::GetErrno() == ENOENT), "unexpected stat error in RenameLogFile");
    }
    
    //rename the file. Use posix rename function
    int result = ::rename(inFileName, theNewNameBuffer);
    if (result == -1)
        theErr = (SInt32)Thread::GetErrno();
    else
        theErr = 0;
...
bool RollingLog::DoesFileExist(const char *inPath)
{
    struct stat theStat;
...
time_t RollingLog::WriteLogHeader(FILE* inFile)
{
   	MutexLocker locker(&fMutex);
...
time_t RollingLog::ReadLogHeader(FILE* inFile)
{
    MutexLocker locker(&fMutex);
...
void RollingLog::ResetToMidnight(time_t* inTimePtr, time_t* outTimePtr) 
{
    if(*inTimePtr == -1)
    {
        *outTimePtr = -1;
    ...
    struct tm  timeResult;
    struct tm* theLocalTime = ::localtime_r(inTimePtr, &timeResult);
