...
UDPSocketPair*  UDPSocketPool::CreateUDPSocketPair(UInt32 inAddr, UInt16 inPort)
{
   //try to find an open pair of ports to bind these suckers tooo
    ...
    while ((!foundPair) && (curPort < kHighestUDPPort))
    {
        ...
        Assert(theElem != NULL);
        if (theElem->fSocketA->Open() != 0)
        {
            this->DestructUDPSocketPair(theElem);
            return NULL;
        }
        if (theElem->fSocketB->Open() != 0)
        {
            this->DestructUDPSocketPair(theElem);
            return NULL;
        }
        ...
        int theErr = theElem->fSocketA->Bind(inAddr, curPort);
        if (theErr == 0)
        {   
            ...
            {   
                ...
                printf("fSocketB->Bind failed on port %u\n", socketBPort);
         }
         else 
            printf("fSocketA->Bind failed on port %u\n", curPort);
