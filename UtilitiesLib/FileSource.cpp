...
void FileSource::Set(const char *inPath)
{
    ...
    fFile = open(inPath, O_RDWR | O_CREAT | O_LARGEFILE);
...
int FileSource::FillBuffer(char* ioBuffer, char *buffStart, SInt32 buffIndex)
{
    UInt32 buffSize = fFileMap.GetMaxBufSize();
    UInt64 startPos = (UInt64) buffIndex * (UInt64) buffSize;
    UInt32 readLen = 0;
    
    int theErr = this->ReadFromPos(startPos, buffStart, buffSize, &readLen);   
...
int FileSource::Read(UInt64 inPosition, void* inBuffer, UInt32 inLength, UInt32* outRcvLen)
{ 
...
int FileSource::ReadFromCache(UInt64 inPosition, void* inBuffer, UInt32 inLength, UInt32* outRcvLen)
{ 
   ...
   if (inPosition >= fLength) // eof
    ...
    if (buffIndex > endIndex || endIndex > maxIndex)
    {
        ...
        printf("FileSource::ReadFromCache bad index: buffIndex=%"_S32BITARG_" endIndex=%"_S32BITARG_" maxIndex=%"_S32BITARG_"\n",buffIndex,endIndex,maxIndex);
        printf("FileSource::ReadFromCache inPosition =%qu buffSize = %"_U32BITARG_" index=%"_S32BITARG_"\n",inPosition, fFileMap.GetMaxBufSize(),buffIndex);
#endif
        Assert(0);
    }
   ...
   {    
        ...
        if (fillBuff)
        {
            int theErr = this->FillBuffer( (char *) inBuffer, (char *) buffStart, (SInt32) buffIndex);
            if (theErr != 0)
                return theErr;
            ...
            ) // that's all there is in the file
        {
            ...
            #if FILE_SOURCE_DEBUG
                printf("FileSource::ReadFromCache end of file reached buffIndex=%"_U32BITARG_" buffSize = %"_S32BITARG_" bytesToCopy=%"_U32BITARG_"\n",buffIndex, buffSize,bytesToCopy);
            #endif
...
#if FILE_SOURCE_DEBUG
        //printf("FileSource::ReadFromCache inLength= %"_U32BITARG_" *outRcvLen=%"_U32BITARG_"\n",inLength, *outRcvLen);
#endif

#if FILE_SOURCE_BUFFTEST    
    {   UInt32 outLen = 0;
        int theErr = this->ReadFromPos(inPosition, inBuffer, inLength, &outLen);       
        
        Assert(*outRcvLen == outLen);
        if (*outRcvLen != outLen)
            printf("FileSource::ReadFromCache *outRcvLen != outLen *outRcvLen=%"_U32BITARG_" outLen=%"_U32BITARG_"\n",*outRcvLen,outLen);
            
        for (int i = 0; i < inLength; i++)
        {   if ( ((char*)inBuffer)[i] != testBuff[i])
            {   printf("FileSource::ReadFromCache byte pos %d of %"_U32BITARG_" failed len=%"_U32BITARG_" inPosition=%qu sBuffCount=%"_S32BITARG_"\n",i,inLength,outLen,inPosition,sBuffCount);
                break;
            }
        }
    }
...
int FileSource::ReadFromDisk(void* inBuffer, UInt32 inLength, UInt32* outRcvLen)
{
    #if FILE_SOURCE_BUFFTEST
        printf("FileSource::Read inLength=%"_U32BITARG_" fFile=%d\n",inLength,fFile);
    #endif
    ...
    int rcvLen = 0;
    do {
        rcvLen = ::read(fFile, (char*)inBuffer, inLength);
    } while((rcvLen == -1) && (Thread::GetErrno() == EINTR));
...
int FileSource::ReadFromPos(UInt64 inPosition, void* inBuffer, UInt32 inLength, UInt32* outRcvLen)
{   
#if TEST_TIME
    {   
        ...
        if (outRcvLen)
            *outRcvLen = 0;
        printf("FileSource::Read sReadCount = %"_S32BITARG_" totalbytes=%"_S32BITARG_" readsize=%"_U32BITARG_"\n",sReadCount,sByteCount,inLength);
    }
#endif

    this->Seek(inPosition);
    int err =  this->ReadFromDisk(inBuffer, inLength, outRcvLen);
...
int FileSource::WriteToDisk(void* inBuffer, UInt32 inLength, UInt32* outSndLen)
{
#if FILE_SOURCE_BUFFTEST
        printf("FileSource::Write inLength=%"_U32BITARG_" fFile=%d\n", inLength, fFile);
#endif
    ...
    int sndLen = 0; 
    do {
        sndLen = ::write(fFile, (char*)inBuffer, inLength);
    } while((sndLen == -1) && (Thread::GetErrno() == EINTR));
...
#if READ_LOG
    fTrackID = trackID;
//  printf("FileSource::SetTrackID = %"_U32BITARG_" this=%"_U32BITARG_"\n",fTrackID,(UInt32) this);
#endif
}
...
void FileSource::Close()
{
    ...
    {   
        sMovie = 0;
//      printf("FileSource::Close sReadCount = %"_S32BITARG_" totalbytes=%"_S32BITARG_"\n",sReadCount,sByteCount);
//      printf("FileSource::Close durationTime = %qd\n",durationTime);
    }
#endif
}
